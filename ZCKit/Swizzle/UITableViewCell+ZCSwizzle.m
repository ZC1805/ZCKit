...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL sel1x = @selector(swizzle1_cell_hitTest:withEvent:);
        zc_swizzle_exchange_instance_selector(UITableViewCell.class, sel1, sel1x);
    });
}

- (UIView *)swizzle1_cell_hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    UIView *hit = [self swizzle1_cell_hitTest:point withEvent:event];
    if (hit) {
