...
- (NSString *)descriptionWithLocale:(id)locale {
    if (self.jsonString.length) return self.jsonString;
...
- (BOOL)keyIsInvalidKey:(NSString *)key {
    ...
    NSAssert(0, @"ZCKit: parse dic or key is fail");
    return YES;
}
    ...
    if ([value isKindOfClass:[NSString class]]) {
        if (allowNull) {
            ...
            if (allowRemove) {
                ...
                if (ZCKitBridge.isPrintLog) NSLog(@"ZCKit: dic inject string obj is invalid");
            }
        }
    } else {
        [self setObject:value forKey:key];
...
- (BOOL)keyIsInvalidKey:(NSString *)key value:(id)value allowNull:(BOOL)allowNull allowRemove:(BOOL)allowRemove {
    ...
    if (value == nil || value == NULL) {
        if (allowRemove) {
            ...
            if (ZCKitBridge.isPrintLog) NSLog(@"ZCKit: dic inject value obj is nil");
        }
        return YES;
    }
    return NO;
