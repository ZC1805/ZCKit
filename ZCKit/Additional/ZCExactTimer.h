...
typedef NS_ENUM(NSUInteger, ZCEnumExactTimerOption) {
    ZCEnumExactTimerOptionAbandon = 0,  /**< 废除之前的同名的任务 */
    ZCEnumExactTimerOptionMerge   = 1,  /**< 合并之前的同名的任务，interval以当前的为准 */
};

@interface ZCExactTimer : NSObject  /**< 精确通用计时器 */

/**
 *  启动一个timer，默认精度为0.01s，不会立即执行，至少会执行一次，repeat为NO时或stop且只有一个block时会自动销毁。
