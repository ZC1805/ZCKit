...
+ (void)scheduledTimer:(NSString *)timerName interval:(NSTimeInterval)interval block:(void(^)(BOOL *stop))block {
    [self scheduledTimer:timerName interval:interval queue:nil repeat:YES attach:nil option:ZCEnumExactTimerOptionMerge block:block];
}
...
- (void)scheduledGlobalTimer:(void(^)(BOOL *stop))block {
    if (!self) {NSAssert(0, @"ZCKit: self is nil object"); return;}
    [ZCExactTimer scheduledTimer:zc_global_timer interval:1.0 queue:nil repeat:YES attach:self option:ZCEnumExactTimerOptionMerge block:block];
}
