...
+ (NSString *)resourcePath:(NSString *)bundle name:(NSString *)name ext:(NSString *)ext {
    ...
    if (!path) path = [bdl pathForResource:[name stringByAppendingString:[ZCGlobal global].imageSuffix] ofType:ext];
    if (!path) path = [bdl pathForResource:[name stringByAppendingString:[ZCGlobal global].imageSuffixSmaller] ofType:ext];
    return path;
}
